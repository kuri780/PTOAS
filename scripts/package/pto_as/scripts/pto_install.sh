...
setenv() {
  logandprint "[INFO]: Set the environment path [ export ASCEND_PTO_AS_PATH=${relative_path_val}/${PTO_PLATFORM_DIR} ]."
...
create_tbe_kernel_softlink() {
  ...
  local dir_mode=""
  local dst_path=${TARGET_INSTALL_PATH}/cann/pto_as/built-in/op_impl/ai_core/tbe/kernel
  if [ -d "${dst_path}" ]; then
    dir_mode=$(stat -c %a ${dst_path})
    if [ "$(id -u)" != 0 ] && [ ! -w "${dir_mode}" ]; then
      chmod u+w "${dst_path}" 2>/dev/null
...
create_tbe_impl_softlink() {
  local dir_mode=""
  local dst_path=${TARGET_INSTALL_PATH}/cann/pto_as/built-in/op_impl/ai_core/tbe/impl
  if [ -d "${dst_path}" ]; then
    dir_mode=$(stat -c %a ${dst_path})
    if [ "$(id -u)" != 0 ] && [ ! -w "${dir_mode}" ]; then
      chmod u+w "${dst_path}" 2>/dev/null
...
create_tbe_softlink() {
  local dir_mode=""
  local dst_path=${TARGET_INSTALL_PATH}/cann/pto_as/built-in/op_impl/ai_core/tbe
  if [ -d "${dst_path}" ]; then
    dir_mode=$(stat -c %a ${dst_path})
    if [ "$(id -u)" != 0 ] && [ ! -w "${dir_mode}" ]; then
      chmod u+w "${dst_path}" 2>/dev/null
...
#create softlink cann/pto_as/built-in
create_latest_builtin_softlink() {
  local dir_mode=""
  local dst_path=${TARGET_INSTALL_PATH}/cann/pto_as/built-in
  if [ -d "${dst_path}" ]; then
    dir_mode=$(stat -c %a ${dst_path})
    if [ "$(id -u)" != 0 ] && [ ! -w "${dir_mode}" ]; then
      chmod u+w "${dst_path}" 2>/dev/null
...
#create softlink cann/pto_as/lib64
create_latest_lib_softlink() {
  local dir_mode=""
  local dst_path=${TARGET_INSTALL_PATH}/cann/pto_as/lib64
  if [ -d "${dst_path}" ]; then
    dir_mode=$(stat -c %a ${dst_path})
    if [ "$(id -u)" != 0 ] && [ ! -w "${dir_mode}" ]; then
      chmod u+w "${dst_path}" 2>/dev/null
...
#create softlink cann/pto_as/include
create_latest_include_softlink() {
  local dir_mode=""
  local dst_path=${TARGET_INSTALL_PATH}/cann/pto_as/include
  if [ -d "${dst_path}" ]; then
    dir_mode=$(stat -c %a ${dst_path})
    if [ "$(id -u)" != 0 ] && [ ! -w "${dir_mode}" ]; then
      chmod u+w "${dst_path}" 2>/dev/null
...
#create softlink cann/pto_as
create_latest_pto_softlink() {
  local dir_mode=""
  local dst_path=${TARGET_INSTALL_PATH}/cann/pto_as/
  if [ -d "${dst_path}" ]; then
    dir_mode=$(stat -c %a ${dst_path})
    if [ "$(id -u)" != 0 ] && [ ! -w "${dir_mode}" ]; then
      chmod u+w "${dst_path}" 2>/dev/null
...
main() {
  ...
  log_with_errorlevel "$?" "error" "[ERROR]: ERR_NO:${INSTALL_FAILED};ERR_DES:Change pto ownership failed.."
