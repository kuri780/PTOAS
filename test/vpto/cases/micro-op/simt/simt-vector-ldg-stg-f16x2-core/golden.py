...
LANES = 2
OPS_PER_LANE = 2
ELEMS = LANES * OPS_PER_LANE * 6  # 3 regions × 2 vectors × 2 halfs = 12 vectors = 24 halfs


def generate(output_dir: Path) -> None:
    output_dir.mkdir(parents=True, exist_ok=True)
    v1 = np.full(ELEMS, -1.0, dtype=np.float16)
    golden_v1 = np.full(ELEMS, -1.0, dtype=np.float16)
    ...
    for lane in range(LANES):
        ...
        a0_vals = np.array([1.0 + lane * 10, 2.0 + lane * 10], dtype=np.float16)
        a1_vals = np.array([3.0 + lane * 10, 4.0 + lane * 10], dtype=np.float16)

        # Fill input B with known values
        b0_vals = np.array([0.5 + lane, 1.5 + lane], dtype=np.float16)
        b1_vals = np.array([2.5 + lane, 3.5 + lane], dtype=np.float16)
