...
def main():
    ...
    golden = np.fromfile("golden_v1.bin", dtype=np.uint16)
    out = np.fromfile("v1.bin", dtype=np.uint16)
    ok = golden.shape == out.shape and np.array_equal(golden, out)
    if not ok:
        idxs = np.nonzero(golden != out)[0]
        idx = int(idxs[0]) if idxs.size else 0
        print(
            f"[ERROR] mismatch at idx={idx}, golden=0x{int(golden[idx]):04x}, out=0x{int(out[idx]):04x}"
        )
        if strict:
            sys.exit(2)
