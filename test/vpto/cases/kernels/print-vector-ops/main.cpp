...
// case: kernels/print-vector-ops — vector vadd (self=2x) + print
...
void LaunchPrint_vector_ops_kernel_mix_aiv(float *v1, float *v2, float v3, float v4, int32_t v5, void *stream);

int main() {
    constexpr size_t kElemCount = 64;
    constexpr size_t kFileSize = kElemCount * sizeof(float);
    float scaleHost = 2.0f;   // for golden: output = input * scale
    float biasHost  = 0.5f;   // (unused in compute but printed)
    ...
    LaunchPrint_vector_ops_kernel_mix_aiv(v1Device, v2Device, scaleHost, biasHost, nHost, stream);
