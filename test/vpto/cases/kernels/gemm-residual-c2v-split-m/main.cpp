...
void LaunchGemm_residual_c2v_split_m_kernel(
...
int main() {
  ...
  constexpr size_t kRowsPerSb = kM / 2;  // 8 rows per subblock
  ...
  constexpr size_t residualHalfSize = kRowsPerSb * kN * sizeof(bf16_storage_t);
  constexpr size_t outHalfSize = kRowsPerSb * kN * sizeof(float);
  ...
  // Compute subblock-specific offsets into the single contiguous buffers after allocation.
  // We declare these before ACL_CHECK to avoid the goto-jump-bypass warning.
  ...
  FILE_CHECK(ReadFile("./v3.bin", inputSize, residualHost, residualSize) && inputSize == residualSize,
             "./v3.bin");
  inputSize = outSize;
  FILE_CHECK(ReadFile("./v4.bin", inputSize, outHost, outSize) && inputSize == outSize,
             "./v4.bin");
  ...
  // Set up subblock-specific pointers into the contiguous buffers
  residualDev0 = residualDevice;
  residualDev1 = residualDevice + kRowsPerSb * kN;
  outDev0 = outDevice;
  outDev1 = outDevice + kRowsPerSb * kN;

  LaunchGemm_residual_c2v_split_m_kernel(
      aDevice, bDevice, residualDev0, residualDev1, outDev0, outDev1, stream);
  ACL_CHECK(aclrtSynchronizeStream(stream));

  // Read back the entire contiguous output buffer and write as single file
  ACL_CHECK(aclrtMemcpy(outHost, outSize, outDevice, outSize, ACL_MEMCPY_DEVICE_TO_HOST));
  FILE_CHECK(WriteFile("./v4.bin", outHost, outSize), "./v4.bin");
