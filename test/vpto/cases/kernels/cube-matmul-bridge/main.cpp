...
void LaunchCube_matmul_bridge_kernel(__fp16 *a, __fp16 *b, float *c,
                                     void *stream);
...
int main() {
  ...
  if (const char *envDevice = std::getenv("ACL_DEVICE_ID")) {
    char *end = nullptr;
    long parsed = std::strtol(envDevice, &end, 10);
    if (end != envDevice && *end == '\0' && parsed >= 0 && parsed <= INT_MAX) {
  ...
  inputSize = aSize;
  FILE_CHECK(ReadFile("./v1.bin", inputSize, aHost, aSize) &&
                 inputSize == aSize,
             "./v1.bin");
  inputSize = bSize;
  FILE_CHECK(ReadFile("./v2.bin", inputSize, bHost, bSize) &&
                 inputSize == bSize,
             "./v2.bin");
  inputSize = cSize;
  FILE_CHECK(ReadFile("./v3.bin", inputSize, cHost, cSize) &&
                 inputSize == cSize,
             "./v3.bin");
  ...
  LaunchCube_matmul_bridge_kernel(aDevice, bDevice, cDevice, stream);
