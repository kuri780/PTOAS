...
# target_ops: BF16 GEMM (tile dialect) + BF16 Residual Convert + FP32 Add
# shapes: A[16,64] B[64,256] Residual[16,256] Output[16,256]
...
def to_bf16_bits(values: np.ndarray) -> np.ndarray:
    """Convert FP32 values to BF16 bit patterns stored as uint16."""
    f32 = values.astype(np.float32, copy=False)
    return (f32.view(np.uint32) >> 16).astype(np.uint16)


def bf16_bits_to_f32(bits: np.ndarray) -> np.ndarray:
    """Convert BF16 uint16 bit patterns back to FP32."""
    return (bits.astype(np.uint32) << 16).view(np.float32)


def generate(output_dir: Path) -> None:
    # Use deterministic but diverse patterns (positive, negative, non-integer)
    # A: FP32 values constructed to exercise BF16 conversion
    row = np.arange(M, dtype=np.float32).reshape(M, 1)
    col = np.arange(K, dtype=np.float32).reshape(1, K)
    # Mix of positive/negative/non-integer values
    ...
    # Residual: different pattern from A, includes strong negative/positive
    ...
    # Output placeholder (will be overwritten by kernel)
    ...
    # v4.bin: Output placeholder [M,N] FP32 (all zeros)
